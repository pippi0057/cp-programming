...
auto Main() -> void {
    ...
    mint ans = 0;
    rep(18){
        if(mow(10, i + 1) <= L) continue;
        if(mow(10, i) > R) continue;
        mint tl = mow(10, i), tr = mow(10, i + 1) - 1;
        if(mow(10, i) <= L && L < mow(10, i + 1)) tl = L;
        if(mow(10, i) <= R && R < mow(10, i + 1)) tr = R;
        ans += mint(i + 1) * ((tr * (tr + 1) / 2) - (tl * (tl - 1) / 2));
        // cout << ans << endl;
        // cout << tl << " " << tr << endl;
    }
    if(R == mow(10, 18)) ans += mint(19) * mint(mow(10, 18));
    cout << ans.val() << endl;
}
