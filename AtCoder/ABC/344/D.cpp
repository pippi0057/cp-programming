...
void Main(){
    ...
    vector dp(N + 1, vector(T.size() + 1, 500));
    dp[0][0] = 0;
    rep(N){
        int m; cin >> m;
        vector<string> v(m); cin >> v;
        rep(j, T.size() + 1) chmin(dp[i + 1][j], dp[i][j]);
        rep(j, T.size() + 1){
            string tmp = T.substr(0, j);
            for(const auto& s : v){
                if(j + s.size() > T.size()) continue;
                if(tmp + s == T.substr(0, tmp.size() + s.size())){
                    chmin(dp[i + 1][j + s.size()], dp[i][j] + 1);
                }
            }
        }
    }
    ...
    //     cout << endl;
    // }
    cout << (dp[N][T.size()] < 500 ? dp[N][T.size()] : -1) << endl;
}
