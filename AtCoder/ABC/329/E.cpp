...
void Main(){
    ...
    while(right - left > 1){
        bool ng = true;
        while(right - left + 1 >= M and S[left] == T[0]) left++;
        while(right - left + 1 >= M and S[right] == T[M - 1]) right--;
        rep(min(left + 1, M)){
            if(S[left] == T[i]){
                bool ok = true;
                rep(j, M - i) if(S[left + j] != T[i + j]) ok = false;
                if(ok){
                    left += M - i;
                    ng = false;
                }
        ...
        rep(min(N - right, M)){
            if(S[right] == T[M - 1 - i]){
                bool ok = true;
                rep(j, M - i) if(S[right - j] != T[M - 1 - i - j]) ok = false;
                if(ok){
                    right -= M - i;
                    ng = false;
                }
