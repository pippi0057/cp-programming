...
void Main(){
    ...
    for(auto& [x, y] : data) cin >> x >> y, y += x;
    sort(all(data), [](const auto& a, const auto& b){ return a.second < b.second; });
    for(const auto& [x, y] : data) cout << x << " " << y << endl;
    int i = 0, now = data[0].first;
    while(true){
        cout << i << endl;
        cout << data[i].first << " " << data[i].second << endl;
        while(data[i].second < now && i != N) i++;
        cout << i << endl;
        if(i == N) break;
        now = data[i].second + 1;
        ans++;
