...
void Main(){
    int N, Q; cin >> N;
    vector<pair<int, int>> events, query;
    rep(N){
        int input; cin >> input;
        events.push_back({input, i % 2});
    }
    cin >> Q;
    while(Q--){
        int l, r; cin >> l >> r;
    ...
    int tmp = 0;
    rep(events.size()){
        auto [now, cmd] = events[i];
        if(data.find(now) != data.end()) continue;
        auto itr = data.end(); --itr;
        if(tmp) data[now] = data[itr->first] + now - itr->first;
        else data[now] = data[itr->first];
        if(cmd == 0) tmp = 0;
        if(cmd == 1) tmp = 1;
    }
    for(const auto& [l, r] : query) cout << data[r] - data[l] << endl;
}
