...
void Main(){
    ...
    for(int i = N; i >= 1; i--) a.push_back({i, 0});
    while(Q--){
        int cmd; cin >> cmd;
        if(cmd - 1){
            int p; cin >> p;
            ...
            char c; cin >> c;
            auto [nx, ny] = a[a.size() - 1];
            if(c == 'R') a.push_back({nx + 1, ny});
            if(c == 'L') a.push_back({nx - 1, ny});
            if(c == 'U') a.push_back({nx, ny + 1});
            if(c == 'D') a.push_back({nx, ny - 1});
        }
    }
}
