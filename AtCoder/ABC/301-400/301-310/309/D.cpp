...
void Main(){
    ...
    rep(M){
        int a, b; cin >> a >> b;
        if(a <= N1 || b <= N1){
            A[a - 1].emplace_back(b - 1, 1);
            A[b - 1].emplace_back(a - 1, 1);
        }else{
            B[a - 1 - N1].emplace_back(b - 1 - N1, 1);
            B[b - 1 - N1].emplace_back(a - 1 - N1, 1);
        }
    }
    auto d1 = dijkstra(A, 0), d2 = dijkstra(B, N2 - 1);
    ll a = 0, b = 0;
    for(const auto& x : d1) chmax(a, x);
    for(const auto& x : d2) chmax(b, x);
    cout << a + b + 1 << endl;
}
