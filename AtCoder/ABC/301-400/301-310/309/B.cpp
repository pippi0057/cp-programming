...
void Main(){
    ...
    rep(N) rep(j, N){
        if(i != 0 && j != 0 && i != N - 1 && j != N - 1) ans[i][j] = A[i][j];
        else if(i == 0 && j != 0) ans[i][j] = A[i][j - 1];
        else if(j == 0 && i != N - 1) ans[i][j] = A[i + 1][j];
        else if(i == N - 1 && j != N - 1) ans[i][j] = A[i][j + 1];
        else if(j == N - 1 && i != 0) ans[i][j] = A[i - 1][j];
    }
    // cout << 1 << endl;
