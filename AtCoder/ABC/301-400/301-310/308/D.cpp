...
void Main(){
    ...
    rep(H) rep(j, W){
        rep(k, 4){
            ...
            if(S[i][j] == 's' && S[nx][ny] == 'n') g[i * W + j].emplace_back(nx * W + ny, 1);
            if(S[i][j] == 'n' && S[nx][ny] == 'u') g[i * W + j].emplace_back(nx * W + ny, 1);
            if(S[i][j] == 'u' && S[nx][ny] == 'k') g[i * W + j].emplace_back(nx * W + ny, 1);
            if(S[i][j] == 'k' && S[nx][ny] == 'e') g[i * W + j].emplace_back(nx * W + ny, 1);
            if(S[i][j] == 'e' && S[nx][ny] == 's') g[i * W + j].emplace_back(nx * W + ny, 1);
        }
    }
    auto dist = dijkstra(g, 0);
    if(dist[H * W - 1] == inf) cout << "No" << endl;
    else cout << "Yes" << endl;
}
