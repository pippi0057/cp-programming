...
void Main(){
    ...
    vector A(N, 0);
    cin >> A >> S;
    auto rle = RLE::encode(S);
    int m = rle.size(), cnt = 0;
    vector data(m, vector(3, 0));
    rep(m) rep(j, rle[i].second){ data[i][A[cnt]]++; cnt++; }
    map<char, vector<int>> mp;
    mp['M'] = {0, 0, 0}; mp['E'] = {0, 0, 0}; mp['X'] = {0, 0, 0};
    rep(m) rep(j, 3) mp[rle[i].first][j] += data[i][j];
    rep(m){
        mp[rle[i].first][0] -= data[i][0];
        mp[rle[i].first][1] -= data[i][1];
        mp[rle[i].first][2] -= data[i][2];
        if(rle[i].first == 'M'){
            ans += data[i][0] * mp['E'][0] * mp['X'][0];
            ans += data[i][0] * mp['E'][0] * mp['X'][1] * 2;
            ans += data[i][0] * mp['E'][0] * mp['X'][2];
            ans += data[i][0] * mp['E'][1] * mp['X'][0] * 2;
            ans += data[i][0] * mp['E'][1] * mp['X'][1] * 2;
            ans += data[i][0] * mp['E'][1] * mp['X'][2] * 3;
            ans += data[i][0] * mp['E'][2] * mp['X'][0];
            ans += data[i][0] * mp['E'][2] * mp['X'][1] * 3;
            ans += data[i][0] * mp['E'][2] * mp['X'][2];
            ans += data[i][1] * mp['E'][0] * mp['X'][0] * 2;
            ans += data[i][1] * mp['E'][0] * mp['X'][1] * 2;
            ans += data[i][1] * mp['E'][0] * mp['X'][2] * 3;
            ans += data[i][1] * mp['E'][1] * mp['X'][0] * 2;
            ans += data[i][1] * mp['E'][2] * mp['X'][0] * 3;
            ans += data[i][2] * mp['E'][0] * mp['X'][0];
            ans += data[i][2] * mp['E'][0] * mp['X'][1] * 3;
            ans += data[i][2] * mp['E'][0] * mp['X'][2];
            ans += data[i][2] * mp['E'][1] * mp['X'][0] * 3;
            ans += data[i][2] * mp['E'][2] * mp['X'][0];
        }
    }
    cout << ans << endl;
}
