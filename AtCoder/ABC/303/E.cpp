...
void Main(){
    ...
    while(tasks.size()){
        int now = tasks.front(); tasks.pop();
        int par = g[now][0];
        if(seen[par]) par = g[now][1];
        seen[now] = 1; seen[par] = 1;
        ans.emplace_back(g[par].size());
        for(const auto& x : g[par]){
            if(g[x].size() - 1){
                int nxt = g[x][0] + g[x][1] - par;
                if(!seen[nxt]) tasks.push(nxt);
            }
            seen[x] = 1;
        }
    }
    for(const auto& x : ans | sorted) cout << x << " ";
    cout << endl;
}
