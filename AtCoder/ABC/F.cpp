...
void Main(){
    ...
    rep(N){
        ...
        rep(j, n){
            int input; cin >> input;
            g[i].push_back({N + input, 0});
            g[N + input].push_back({i, 1});
            if(input == 1) a.insert(i);
            if(input == M) b.insert(i);
        }
    }
    auto dijkstra = [a, b, &ans](const Graph& g, int s) -> void {
        int n = g.size();
        vector<ll> dist(n, inf);
        priority_queue<pair<ll,int>,vector<pair<ll,int>>,greater<pair<ll,int>>> task;
        dist[s] = 0;
    ...
    for(const auto& x : a) dijkstra(g, x);
