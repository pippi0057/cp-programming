...
void Main(){
    int N; cin >> N;
    vector<vector<string>> ans(N, vector<string>(N, "T"));
    int cmd = 0, nx = 0, ny = -1;
    for(int i = 1; i <= N * N - 1; i++){
        if(cmd == 0){
            ny++;
            ans[nx][ny] = to_string(i);
            if(ny == N - 1 || ans[nx][ny + 1] != "T"){
                cmd = 1;
                continue;
            }
        }
