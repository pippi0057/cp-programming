...
auto Main() -> void {
    int N, Wsum = 0; cin >> N;
    vector<ll> W(N, 0), H(N, 0), B(N, 0);
    rep(N){
        cin >> W[i] >> H[i] >> B[i];
        Wsum += W[i];
    }
    vector<vector<ll>> dp(N + 1, vector<ll>(500 * 500, 0));
    for(const auto& x : B) dp[0][0] += x;
    rep(N){
        rep(j, 500 * 500){
            chmax(dp[i + 1][j], dp[i][j]);
            if(j + W[i] < 500 * 500) chmax(dp[i + 1][j + W[i]], dp[i][j] + H[i] - B[i]);
        }
    }
    ll ans = 0;
    rep((Wsum / 2) + 1) chmax(ans, dp[N][i]);
    cout << ans << endl;
}
